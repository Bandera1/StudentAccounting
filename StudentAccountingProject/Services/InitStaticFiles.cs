                                               ...
                                               string settingfileName, string base64)
        {
            ...
            try
            {
                ...
                foreach (var imagePrefix in imageSizes)
                {
                    ...
                    if (bmp != null)
                    {
                        var image = ImageService.CompressImage(bmp, size, size);
            ...
            try
            {
                ...
                using (var bmp = base64.FromBase64StringToImage())
                {
                    ...
                    foreach (var imagePrefix in imageSizes)
                    {
                        ...
                        if (bmp != null)
                        {
                            using (var image = ImageService.CompressImage(bmp, size, size))
                            {
                                if (image == null)
                                    throw new Exception("There were problems in the process of creating a photo");
